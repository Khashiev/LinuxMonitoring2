...
function check_parameter {
    if [[ $(check_number $1) -eq 1 ]] && [[ $1 -gt 0 ]] && [[ $1 -lt 4 ]]; then
        echo "1"
    else
        print_error "2"
        echo "0"
...
function check_input_date {
    ...
    if [[ $(check_day ${input_date[0]}) -eq 1 ]] && [[ $(check_month ${input_date[1]}) -eq 1 ]] &&
        ...
        [[ $(check_min ${input_date[4]}) -eq 1 ]]; then
        if [[ $(check_days_in_month "${input_date[0]} ${input_date[1]} ${input_date[2]}") -eq 1 ]]; then
            echo "1"
        else
            print_error "4"
            echo "0"
