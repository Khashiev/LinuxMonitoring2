...
function get_date {
    input_date="$1"
    if [[ $(echo "$input_date" | grep -E "^[0-9]+\.[0-9]+\.[0-9]+ [0-9]+\:[0-9]+$") ]]; then
...
if [[ $output -ne 0 ]]; then
    ...
    if [[ $(($end_date_in_sec - $start_date_in_sec)) -lt 0 ]]; then
        output=0
        print_error "5"
    fi
fi
...
function clean {
    for file in $(sudo find / -maxdepth 1 2>/dev/null); do
        if [[ $(compare_name $file) -eq 1 ]]; then
            for item in $(sudo find $file -newermt "${start_date[2]}-${start_date[1]}-${start_date[0]} ${start_date[3]}:${start_date[4]}:00" -and ! -newermt "${end_date[2]}-${end_date[1]}-${end_date[0]} ${end_date[3]}:${end_date[4]}:00" 2>/dev/null); do
                if [[ $item = *"_$current_date"* ]]; then
