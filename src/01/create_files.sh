...
name_file=$(echo $name | awk -F"." '{ print $1 }')
file_extension=$(echo $name | awk -F"." '{ print $2 }')
...
size=$6
file_size=$(echo "$size" | awk -F"kb" '{print $1}')
#####

size_disk=$(sudo df -BG / | grep / | awk '{ printf $4 }' | sed s/"G"//g)
...
function create_files {
    ...
    while [[ count -gt 0 ]] && [[ $size_disk -gt 1 ]]; do
        if [[ -f "${input_path}${name_file}_${current_date}.${file_extension}" ]] || [[ $(sum_array ${array_file[@]}) -lt 4 ]]; then
            ...
            sudo dd if=/dev/zero of="${input_path}${name_file}_${current_date}.${file_extension}" bs="${file_size}KB" count=1 status=none
            echo -e "${input_path}${name_file}_${current_date}.${file_extension}\t$(date +%d.%m.%Y)\t${file_size}K" >>./log.txt
            array_file=($(iter_array ${array_file[@]}))
            name_file=$(create_name_file ${array_file[@]})
            count=$(($count - 1))
            size_disk=$(sudo df -BG / | grep / | awk '{ printf $4 }' | sed s/"G"//g)
...
while [[ num_dirs -gt 0 ]] && [[ $size_disk -gt 1 ]]; do
    if [[ -d "${path}${name_dir}_${current_date}" ]] || [[ $(sum_array ${array_dir[@]}) -lt 4 ]]; then
        ...
        mkdir "${path}${name_dir}_${current_date}"
        echo -e "${path}${name_dir}_${current_date}\t$(date +%d.%m.%Y)" >>./log.txt
