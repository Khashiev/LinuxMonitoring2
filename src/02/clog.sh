...
for dir in $(sudo find $path -type d 2>/dev/null); do
    if [[ $(compare_name $dir) -eq 1 ]]; then
        num_dirs=$(random)
        num_files=$(random)
        ./create_files.sh $dir $num_dirs $name_dir $num_files $name_file $file_size
